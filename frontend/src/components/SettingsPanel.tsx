...
import { bgOptions, setBgStyle } from './BackgroundLayer';
...
export default function SettingsPanel() {
  ...
  const [currentBg, setCurrentBg] = useState<string>(() => {
    try { return localStorage.getItem('ether:bgStyle') || 'NoBg'; } catch { return 'NoBg'; }
  });
  ...
  async function sendRelay(state: boolean) {
    ...
    try {
      await fetch(`${API_BASE}/api/relay`, {
        method: 'POST',
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/config`).then(r => r.json()).then((d: { config?: {
  ...
  return (
    <div className="settings-root">
      ...
      {/* ── Appearance ───────────────────────────────────────────────────────── */}
      <div className="settings-card">
        <SectionTitle icon="fa-palette" title="Background Style" />
        <p className="settings-desc">
          Choose a background for the dashboard. Your preference is saved locally in the browser.
        </p>
        <div className="bg-selector-grid">
          {bgOptions.map(opt => (
            <button
              key={opt.id}
              className={`bg-selector-card ${currentBg === opt.id ? 'active' : ''}`}
              onClick={() => applyBg(opt.id)}
            >
              <span className="bg-selector-icon">
                {opt.id === 'NoBg'      && <i className="fa-solid fa-ban" />}
                {opt.id === 'DotGrid'   && <i className="fa-solid fa-braille" />}
                {opt.id === 'Aurora'    && <i className="fa-solid fa-wand-magic-sparkles" />}
                {opt.id === 'GridLines' && <i className="fa-solid fa-table-cells" />}
                {!['NoBg','DotGrid','Aurora','GridLines'].includes(opt.id) && <i className="fa-solid fa-image" />}
              </span>
              <span className="bg-selector-label">{opt.label}</span>
              {currentBg === opt.id && <i className="fa-solid fa-check bg-selector-check" />}
            </button>
          ))}
        </div>
      </div>
