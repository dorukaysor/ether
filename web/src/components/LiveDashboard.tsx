...
export default function LiveDashboard() {
  ...
  // ── Fetch — single dispatch = single re-render per cycle ─────────────────
  const fetchData = useCallback(async () => {
    let res: ApiResponse;
    if (mockMode === 'off') {
      try { res = await fetch('/api/readings').then(r => r.json()) as ApiResponse; }
      catch { dispatch({ type: 'ERROR', msg: 'Could not reach ETHER unit.' }); return; }
  ...
  return (
    <div className="live-dashboard">
      ...
      {/* ── Hero row: big power + session stats ── */}
      <div className="dash-hero-row">

        {/* Hero power card */}
        <div className="hero-power-card" style={{ '--sc': sparkColor } as React.CSSProperties}>
          <div className="hpc-header">
            <span className="hpc-label">
              <span className="live-dot" style={{ background: sparkColor } as React.CSSProperties} />
              Live Power
            </span>
            {loading
              ? <span className="hpc-meta">—</span>
              : <span className="hpc-meta">
                  {readings?.frequency.toFixed(1)} Hz &nbsp;·&nbsp; PF {readings?.power_factor.toFixed(2)}
                </span>
            }
          </div>
          <div className="hpc-value-row">
            {loading
              ? <div className="skeleton-box" style={{ width: 200, height: 58, borderRadius: 8 }} />
        ...
        <div className="session-panel">
          <div className="sp-row">
            <span className="sp-label"><i className="fa-solid fa-stopwatch" /> Uptime</span>
            <UptimeClock start={sessionStart.current} />
          </div>
          <div className="sp-sep" />
          <div className="sp-row">
            <span className="sp-label"><i className="fa-solid fa-bolt" /> Energy</span>
          ...
          <div className="sp-sep" />
          <div className="sp-row">
            <span className="sp-label"><i className="fa-solid fa-fire" /> Peak</span>
          ...
          <div className="sp-sep" />
          <div className="sp-row">
            <span className="sp-label"><i className="fa-solid fa-signal" /> Source</span>
